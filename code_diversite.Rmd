...
p <- ggplot(nmds_scores, aes(x = MDS1, y = MDS2, colour = type, shape = site)) +
  ...
  scale_color_manual(values = c("A" = "#619CFF", "D" = "#F8766D", "DA" = "#44BB99")) +
  ...
  scale_shape_manual(values = c("PIH" = 16, "TIA" = 17, "BOU" = 18, "UNU" = 4, "DIA" = 15, "GRO" = 8)) +
...
# Calculer la distance de Bray-Curtis entre les échantillons
dist_chloe <- vegdist(data[, 7:49], method = "bray", diag = TRUE)
...
# Fusionner les catégories "D" et "DA" en une seule catégorie "D/DA"
nmds_scores$type <- as.character(nmds_scores$type)
nmds_scores$type[nmds_scores$type %in% c("D", "DA")] <- "D/DA"
nmds_scores$type <- factor(nmds_scores$type, levels = c("A", "D/DA"))

# Vérifier le nombre de réplicats par groupe après fusion
table(nmds_scores$type)

# Test de permutation pour NMDS (ANOSIM)
anosim_result <- anosim(dist_chloe, nmds_scores$type)
print(anosim_result)
...
p <- ggplot(nmds_scores, aes(x = MDS1, y = MDS2, colour = type, shape = site)) +
  ...
  labs(title = "NMDS de la structure des communautés d'invertébrés",
       ...
       y = "Dimension 2") +
  theme(legend.position = "right") +
  scale_color_manual(values = c("A" = "#619CFF", "D/DA" = "#F8766D")) +
  scale_shape_manual(values = c("PIH" = 16, "TIA" = 17, "BOU" = 18, "UNU" = 4, "DIA" = 15, "GRO" = 8)) +
  scale_x_continuous(name = "Dimension 1", breaks = seq(-2, 2, by = 0.5), limits = c(-2, 2)) +
  scale_y_continuous(name = "Dimension 2", breaks = seq(-2, 2, by = 0.5), limits = c(-2, 2))
